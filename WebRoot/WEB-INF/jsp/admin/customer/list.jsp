...
<html>
	<body>
		<display:table name="list"
			...
			<display:column title="职务" property="zw"/>
			<display:column title="职务说明" property="zwsm"/>
			<display:column title="状态" property="status"/>
			<display:column title="角色" property="jsName"/>
						<c:choose>
	...
	function updateColumnstatus(status,id){
		...
		//更新字段内容
	function updateColumnjsId(jsId,id){
		if(!confirm("确定要更新为"+jsId+"吗?")){
			return false;
		}
		//var aaa = prompt("请输入操作意见或说明：","");
		var params={id:id,jsId:jsId};
		$.post("updateColumnjsId.html",params,function(
				result){
			result=eval("("+result+")");
			if(result.status=="true"||result.status==true){
				alert('成功');
