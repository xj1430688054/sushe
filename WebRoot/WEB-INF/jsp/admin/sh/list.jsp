...
<html>
	<body>
		<display:table name="list"
			...
			<display:column title="损坏情况" property="shqk"/>
			<display:column title="是否修复" property="isxf"/>
			<display:column title="损坏时间">
				<fmt:formatDate value="${row.beginDate}" type="date" pattern="yyyy-MM-dd HH:mm:ss"/>	</display:column>
			<display:column title="修复时间">
	...
	function updateColumnssId(ssId,id){
		...
		//更新字段内容
	function updateColumnisxf(isxf,id){
		if(!confirm("确定要更新为"+isxf+"吗?")){
			return false;
		}
		//var aaa = prompt("请输入操作意见或说明：","");
		var params={id:id,isxf:isxf};
		$.post("updateColumnisxf.html",params,function(
				result){
			result=eval("("+result+")");
			if(result.status=="true"||result.status==true){
				alert('成功');
