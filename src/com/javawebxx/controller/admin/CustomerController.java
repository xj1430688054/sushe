...
Map customer =getCustomer(request);Map map = db.queryForMap("select * from t_customer where id=?",new Object[]{customer.get("id")});model.addAttribute("map", map);		return "/customer/mine";
	...
	public ResponseEntity<String> mineSave(Model model,HttpServletRequest request,Long id
		...
		int result = 0;
			String sql="update t_customer set customerName=?,phone=?,age=?,sex=?,address=?,email=?,zw=?,zwsm=? where id=?";
			result = db.update(sql, new Object[]{customerName,phone,age,sex,address,email,zw,zwsm,id});
		if(result==1){
